...
export default function RideList() {
  ...
  const [myRequests, setMyRequests] = useState({}); // { [rideId]: { _id, status } }
  ...
  const fetchMyRequests = async () => {
    ...
    try {
      // GET /api/requests/user returns array: [{ _id, status, ride: {...} }]
      const { data } = await axios.get(`${API}/api/requests/user`, {
        headers: { Authorization: `Bearer ${token()}` },
      });
      const map = {};
      (data || []).forEach(r => {
        const rideId = r.ride?._id || r.ride; // populated or id
  ...
  useEffect(() => { fetchRides(); fetchMyRequests(); }, []);
